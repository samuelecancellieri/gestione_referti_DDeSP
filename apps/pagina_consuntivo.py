...
def update_table_referti(unita_operativa):
    # ritorna tabella contenente i link ai file in documenti_accettazione
    conn = sqlite3.connect(database)
    c = conn.cursor()
    tabella_referti = pd.read_sql_query(
    ...
    # tabella_referti = tabella_referti.groupby(['anno', 'strumento'])
    tabella_referti['Positivo_anno'] = ((
        tabella_referti['UFC_batteri'] >= 1) | (tabella_referti['UFC_miceti'] >= 1)).groupby([tabella_referti['strumento'], tabella_referti['strumento']]).transform('sum')
...
def return_layout():
    ...
    layout = html.Div(
        [
            ...
            dbc.Row(
                html.Div(html.H4('Tabella Consuntivo'))
            ),
            dbc.Row(
                dbc.Col(
                    html.Div(
