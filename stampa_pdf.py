...
def stampa_referto(codice_MR,id_accettazione, id_campione, unita_operativa, data_prelievo, data_accettazione, rapporto_di_prova, descrizione_campione, operatore_prelievo_campione, operatore_analisi, data_inizio_analisi, data_fine_analisi, esame_microscopico, coltura, ufc_batteri, ufc_miceti, note):
    ...
    if codice_MR=='MR43':#tabella MR43
        ...
        pdf.text(108,115,'>= 50 UFC/ml soglia intervento')
        pdf.text(108,119,'< 100 UFC/ml (batteri)')
        pdf.text(108,123,'< 10 UFC/ml (miceti filamentosi)')
    elif codice_MR=='MR47':# tabella MR47
        # header table risultati
        pdf.set_left_margin(20)
        pdf.set_text_color(0, 0, 0)  # black
        pdf.set_font('arial', 'B', 10)
        pdf.cell(80, 10, 'ID CAMPIONE', 'TLR', 0, 'C')
        pdf.cell(30, 10, 'UFC/ml', 'TLR', 0, 'C')
        pdf.cell(60, 10, 'Metodo', 'TLR', 1, 'C')
        # risultati
        pdf.set_font('arial', '', 8)
        pdf.cell(80, 15, str(id_campione), 1, 0, 'C')
        pdf.cell(30, 15, str(ufc_batteri), 1, 0, 'C')
        pdf.cell(60, 15, '', 1, 1, 'C')
        pdf.set_font('arial', '', 8)
        pdf.cell(0, 10, 'n.r.*: non rivelato, nessun sviluppo o < 1 UFC', 0, 0, 'L')
        # manually writing of multi row text in cell
        pdf.text(140,115,'Allegato 4 Dossier 133/2006')
        pdf.text(140,119,'"Reprocessing degli endoscopi')
        pdf.text(140,123,' - indicazioni operative"')
        # nota post tabella classe
        pdf.set_left_margin(10)
        #skip line of #mm
        pdf.ln(10)
        ...
        pdf.ln(5)
        pdf.set_font('arial', 'B', 10)
        pdf.cell(50, 10, 'Osservazione microscopica: ', 0, 0, 'L')
        pdf.set_font('arial', '', 10)
